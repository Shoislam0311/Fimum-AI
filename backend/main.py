...
async def upload_file(user_id: str = Form(...), file: UploadFile = File(...)):
    try:
        ...
        storage.create_bucket("uploads", public=True)
        file_bytes = await file.read()
        file_url = f"uploads/{user_id}/{file.filename}"
        storage.from_("uploads").upload(file_url, file_bytes)
        public_url = storage.from_("uploads").get_public_url(file_url)
